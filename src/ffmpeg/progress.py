...
def calculate_real_eta(current_time: float, total_duration: float, speed: float) -> str | None:
    """
...
def parse_ffmpeg_output_for_progress(line: str, total_duration: float | None) -> tuple[float | None, int | None, str, str, str, str | None, str | None]:
    ...
    stats_match = re.search(
        ...
        r'.*?'                         # Пропускаем промежуточные данные
        r'bitrate=\s*([\d\.N/A]+\s*k?bits/s|N/A)\s+' # Bitrate с поддержкой N/A
        r'speed=\s*([\d\.]+)x',        # Speed
        line
    )
    ...
    if stats_match:
        ...
        if speed <= 0 or not total_duration:
            ...
            eta_str = calculate_real_eta(current_time_seconds, total_duration, speed)
