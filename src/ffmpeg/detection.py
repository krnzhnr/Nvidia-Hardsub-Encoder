    ...
    try:
        ...
        if result.returncode == 0:
            ...
            if result.stderr and result.stderr.strip():
                ...
                last_error_line = "(нет вывода stderr)"
            return False, (f"'{nvidia_smi_cmd}' ошибка (код {result.returncode}): "
                            f"{last_error_line}")
...
def detect_nvidia_hardware() -> tuple[dict | None, str]:
    ...
    try:
        ...
        # Фильтр называется 'subtitles', а не 'libass' в списке filters
        if 'subtitles' in results["filters"]:
            messages.append("Фильтр FFmpeg 'subtitles' (для libass) найден.")
            hw_info['subtitles_filter'] = True
        else:
            messages.append(
                "[Предупреждение] Фильтр FFmpeg 'subtitles' (для libass) "
                "не найден. Вшивание субтитров может быть невозможно."
            )
        ...
        nvidia_decoders_candidates = {
            ...
            # av1_cuvid может не существовать, но av1_nvdec - да
            'av1': ['av1_nvdec', 'av1_cuvid'],
            'mpeg1': ['mpeg1_cuvid'],
        ...
        for common_codec_name, potential_decoders in nvidia_decoders_candidates.items():
            for ffmpeg_decoder_name in potential_decoders:
                if ffmpeg_decoder_name in available_decoders_in_ffmpeg:
                    detected_hw_decoders[common_codec_name] = ffmpeg_decoder_name
                    break  # Нашли предпочтительный

        if detected_hw_decoders:
            hw_info['decoder_map'] = detected_hw_decoders
            messages.append(
                f"Доступные HW декодеры NVIDIA: {detected_hw_decoders}"
            )
        else:
            messages.append(
                "[Предупреждение] Аппаратные декодеры NVIDIA (cuvid/nvdec) "
                "не найдены в FFmpeg. Декодирование будет на CPU."
            )

        if hw_info['type'] == 'nvidia' and hw_info['encoder']:
            sub_filter_stat = 'Да' if hw_info['subtitles_filter'] else 'Нет'
            status_msg = (
                f"Выбран режим: Тип={hw_info['type']}, "
                f"Доступные HW дек.: {list(hw_info['decoder_map'].keys())}, "
                f"Энкодер={hw_info['encoder']}, "
                f"Фильтр субтитров: {sub_filter_stat}"
            )
            messages.append(status_msg)
            return hw_info, "\n".join(messages)
        else:
            messages.append(
                "Критическая ошибка: Энкодер NVIDIA не найден, хотя GPU "
                "может присутствовать. Проверьте сборку FFmpeg."
            )
            return None, "\n".join(messages)
        ...
        messages.append(
            f"Непредвиденная ошибка при проверке кодеков/фильтров FFmpeg: {e}"
        )
        return None, "\n".join(messages)